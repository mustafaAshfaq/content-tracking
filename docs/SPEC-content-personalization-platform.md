...
42. As an analyst, I want bots and staff/operator accounts excluded from the experiment, so that results are not polluted by automated or operator traffic.
...
73. As a visitor, I want a no-email local demo signup that mints a server-owned opaque userId and a first-party session, so that I can become a known user without sending PII.
74. As a signed-in demo user, I want to remain eligible for `recommendations_layout`, so that the documented success path (sign in, consent, qualify, see personalized recommendations, inspect attribution) can be measured.
75. As an operator, I want `/admin/attribution` gated by a local admin secret from environment configuration, so that the report is not a public page on localhost.
76. As an analyst, I want every `article_viewed` event to require exactly one taxonomy `category`, so that audience SQL can count distinct articles per category without ambiguity.
77. As an analyst, I want every `recommendation_clicked` event to require `is_personalized`, so that fallback clicks are distinguishable from activated recommendations.
...
`article_viewed` requires a single `category` from the locked taxonomy (`travel` | `finance` |
`health` | `technology`). Do not use a multi-value `category_ids` list as the audience key.
`recommendation_clicked` requires `is_personalized` (true only after a successful authorized
activation that rendered the personalized list). `experiment_exposed` requires both Analytics and
Personalisation consent at dispatch time (Analytics for measurement; Personalisation because
exposure is defined only after personalized render). Early decision text that allowed an anonymous
activation identifier, or that treated `category_ids` as sufficient for audience compute, is
...
Local demo authentication is passwordless and collects no email or other direct identifiers. A
visitor completes an explicit "Continue as demo user" action; the server mints the opaque `userId`,
sets a host-only first-party session cookie (`Path=/`, `SameSite=Lax`, `Secure` under HTTPS), links
the current `anonymous_id`, and may emit `signup_completed` with `signup_method: demo` only when
Marketing consent is granted (the alias link itself is Necessary/session work and does not wait on
Marketing). Returning demo sessions are restored from that cookie. Anonymous visitors never call
`/segments/:userId` or any anonymous activation equivalent.
...
`treatment` presentation variants that leave eligibility/segments/products/ranking unchanged.
Version 1 is open-ended until an environment-configured end timestamp is set; variants and
allocation stay immutable for the version. Exclude bots (well-known crawler user agents) and
staff/operator accounts (accounts flagged as operator, including the local admin principal).
In-app demo signup (`signup_method: demo`) is experiment-eligible — that is the documented
success-path visitor, not an excluded "demo account." Assignment is lazy:
...
panel, and JSON/CSV export, with small-cell suppression (hide rates when a cell has fewer than
five conversions), no direct identifiers, and explicit stale/unavailable states. Access is gated
by a local admin secret from environment configuration (shared secret posted once to establish a
host-only admin session cookie). It is not a public route and is not the demo-user session.

A reporting window is **qualified** only when that window contains zero duplicate
`(event_id, event_version)` rows and zero conversions-before-exposure. Other health counts
(malformed aliases, cross-version assignments, missing consent eligibility, conflicting
first-touch records, broken correlation chains) still appear on the health panel; they do not
silently rewrite source rows.
...
RudderStack OSS is the ingestion and Postgres warehouse loader. Configure a Postgres warehouse
destination in the static workspace config so named track tables (including `article_viewed`) and
identity tables are the surface the audience worker queries. App-owned tables (`identity_aliases`,
`audience_membership`, attribution views) live in an isolated app schema, not in RudderStack's
internal jobs database.

The browser loads a GTM **web** container with consent default-denied. The web container's server
transport URL is the local tagging origin. Checked-in container-export JSON is authoring content;
runtime `CONTAINER_CONFIG` is an env secret and is never committed. Ticket implementation may
author tags/triggers to match this consent and forwarding contract; it must not invent a second
data path around `trackEvent()` / `dataLayer`.

**Already true in this codebase (do not rebuild as a greenfield slice).** The article site,
fixture v1, client recommendation module in fallback-only mode, typed `trackEvent()` boundary with
generated JSON Schema, CSP/CORS headers, Compose service definitions, mock destinations, and
lint/type/unit/schema/fixture/build CI already exist. Remaining work is identity minting and demo
session, Klaro UI and purpose wiring, Compose init/workspace/warehouse files the compose file
already mounts, data-layer forwarding into GTM/RudderStack, audience worker and `/segments/:userId`,
experiment assignment/exposure, UTM capture and `/admin/attribution`, and Playwright/CI tiers
including the Compose stack.

**Implementation order.** Remaining slices, not a greenfield restart: (1) identity cookie, demo
session, Klaro, `article_viewed.category`. (2) Compose init SQL, RudderStack workspace/warehouse
config, bootstrap health. (3) Data-layer forwarding into GTM/RudderStack and mock observability.
(4) Aliasing, audience worker, `/segments/:userId`, personalized module success path. (5)
Experiment assignment/exposure, UTM capture, attribution views, admin report. (6) Playwright/CI
tiers, GTM walkthrough, evidence collection.
...
fallback contents), duplicate-ID rejection, and the production build. Prior art lives in this
repo: existing Vitest coverage of `trackEvent()`, consent fail-closed behaviour, page-view route
keys, recommendation fallback outcomes, CSP/CORS, and fixture validation. Extend those tests;
do not replace them. A repeatable, dated GTM Preview/Tag Assistant walkthrough provides manual
audit evidence layered on top of the automated seams.
...
- This spec is the parent artifact for `/to-tickets` and `/implement`. It synthesizes the closed
  wayfinder tickets in [map.md](map.md). [FINAL-HANDOFF-SPEC.md](FINAL-HANDOFF-SPEC.md) remains the
  detailed companion for HTTP examples, fixture lists, and ports. Where this spec and an older
  decision ticket disagree, this spec wins (anonymous activation is forbidden; demo signup is
  in-experiment; `article_viewed.category` is required; `recommendation_clicked.is_personalized`
  is required; `experiment_exposed` needs Analytics and Personalisation).
...
- A live `GTM_CONTAINER_CONFIG` string cannot be invented in tickets; operators provision it in
  GTM and place it in gitignored env. Tickets may check in export JSON and fail closed when the
  secret is missing.
